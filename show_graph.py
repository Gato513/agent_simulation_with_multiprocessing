...
def build_img_pos(np_, img_w, img_h):
    return {l: norm_to_img_px(x, y, img_w, img_h) for l, (x, y) in np_.items()}  # noqa: E741
...
class NodePicker:
    ...
    def __init__(self, ax, G, pos, radius=0.07, pair=None):
        ...
        self._xy = np.array([pos[l] for l in self._labels], dtype=float)  # noqa: E741
    ...
    def _add_ring(self, ax, pos_dict, label):
        ...
        dot = ax.scatter([x], [y], s=60, c=ACCENT, zorder=16, linewidths=0)
    ...
    def _on_click(self, event):
        ...
        print(
            f"[EVENT] button={event.button} inaxes={event.inaxes is not None} x={_ex} y={_ey}"
        )
...
def draw_graph_on_ax(ax, G, pos, bg=DARK_BG, alpha=0.88):
    ...
    nx.draw_networkx_edges(
        G, pos, ax=ax, edge_color=ec, width=ew, alpha=alpha, arrows=True
    )
...
def draw_legend(ax, compact=False):
    ...
    p1 = [
        mpatches.Patch(
            facecolor=ZONA_COLORS[z],
            edgecolor=ZONA_EDGE_COLORS[z],
            linewidth=0.8,
            label=z.capitalize(),
        )
...
def draw_stats(ax, G, compact=False):
    ...
    ax.text(
        ...
        color=TEXT_CLR,
        bbox=dict(
            boxstyle="round,pad=0.4", facecolor="#1a1e2e", edgecolor=MUTED, alpha=0.92
        ),
    )
...
def draw_hint(target, compact=False, use_fig=False):
    ...
    if use_fig:
        ...
        target.text(
            0.5, 0.005, msg, ha="center", va="bottom", transform=target.transAxes, **kw
        )
...
def mode_solo(G, pos):
    ...
    draw_hint(ax)
    ax.set_title(
        "CIRCUITO COMERCIAL — ENCARNACIÓN",
        color=ACCENT,
        fontsize=14,
        fontweight="bold",
        fontfamily="monospace",
...
def mode_overlay(G, pos):
    ...
    ax.set_title(
        "CIRCUITO COMERCIAL — SUPERPOSICIÓN",
        color=ACCENT,
        fontsize=12,
        fontweight="bold",
        fontfamily="monospace",
...
def mode_dual(G, pos):
    ...
    ax_l.set_title(
        "GRAFO  ·  COORDENADAS NORMALIZADAS",
        color=ACCENT,
        fontsize=10,
        fontweight="bold",
        fontfamily="monospace",
...
def mode_info(G):
    ...
    print("\n  Aristas por vel_max:")
    for v, c in sorted(vels.items()):
        print(f"    {v:>3} km/h → {c:>4}")
...
if __name__ == "__main__":
    ...
    if arg == "--solo":
        mode_solo(G, node_position)
    elif arg == "--overlay":
        mode_overlay(G, node_position)
    elif arg == "--info":
        mode_info(G)
    else:
        mode_dual(G, node_position)
